...
|---|---:|---:|---:|---:|---|---|
|`data`|282|282|282|0|28.6 MiB / 28.6 MiB|覆盖|
|`textures`|111|111|111|0|30.5 MiB / 30.5 MiB|覆盖|
...
|`skycultures`|1297|1290|1290|0|137.5 MiB / 137.5 MiB|覆盖|
|`scripts`|118|52|52|0|7.5 MiB / 4.8 MiB|覆盖|
...
本文件保留为 2026-08-24 的历史快照。之后已完成三维地景资源链路：`scripts/sync-ohos-resources.sh` 现在同步 `scenery3d/`，鸿蒙端通过“更多 > 3D场景”读取原版 `scenery3d.ini`、多语言介绍、作者、版权、模型和观测位置，并调用原版场景加载命令。

最新 HAP 已核验包含 `resources/rawfile/stellarium/scenery3d/` 下的 135 个文件，其中包括 2 个场景配置、65 个多语言介绍文件、OBJ/MTL 模型和纹理。因此，本报告中“scenery3d 未打包、未接入”的内容仅代表 2026-08-24 当时的状态，不能用于判断当前构建。
