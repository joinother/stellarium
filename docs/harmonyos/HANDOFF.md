...
- 响应式适配以窗口宽度/高度断点为准，手机正式面板底部居中，Dock 使用独立的手机字号和稳定触控高度；不得恢复按设备型号分叉业务逻辑。
- 旧 iPad 左侧竖栏/侧栏抽屉已经弃用，不得恢复为新的功能入口。源码中仅保留历史 Builder 的兼容符号，并标记 `@deprecated`；它们不参与当前 Shell 的布局、命中测试和天体避让。

ArkUI、CLI 和 AI 预研的查询与验证流程见 `docs/harmonyos/DEVELOPMENT-MCP-WORKFLOW.md`。当前 API 24 工程不直接使用 API 26 的 `ContainerReader` 或未经验证的系统 AI Kit；新业务先登记 CLI 命令契约，再接入统一 Shell。

> **创建时间：** 2026-07-27
> **最后更新：** 2026-08-31
> **当前分支：** `fix/api22-privacy-v2`
> **当前状态：** 能构建、安装、启动；手机紧凑面板已按窗口断点底部居中，Pad/桌面继续使用统一底部 Dock 和面板状态机
...
- ✅ 浏览分类（太阳系、恒星、星座、深空、人造天体；`ArtificialObjects` 合并人工航天器与离线卫星目录，星云总目录使用 `NebulaMgr:200`）
- ✅ 夜间模式
- ✅ FPS计数器（实时显示渲染帧率）
