...
2026-09-07 复核见 [国产化、分发与资源下载报告](DOMESTIC-RESOURCE-DISTRIBUTION-RESEARCH-2026-09-07.md)。本文的远端可信清单、整批原子激活属于设计目标，不应误读为已实现的下载器能力；当前仍是运行时离线、构建机显式更新。
...
每份资源应同时出现在清单中，至少包含：`sourceId`、`version`、`fetchedAt`、`contentType`、`bytes`、`sha256`、`license`、`attribution` 和 `upstreamReference`。目标流程是构建脚本先认证远端发布清单，再下载文件并核对预期 SHA-256；失败时保留旧资源。当前脚本只在下载后计算哈希并写本地清单，尚未实现远端清单认证/预期哈希比较，不能据此宣称来源可信校验已经完成。

静态目录服务不应额外接收地理经纬度、搜索词、SN、设备序列号或隐私同意状态，但服务器仍能看到 IP、时间和资源路径；巡天瓦片路径可推断天区。实时飞机、在线天体查询和 MPC 查询若未来接入镜像，必须单独评估：这些请求天然携带位置或对象查询，不能把“镜像”当成隐私问题的自动解决方案。
