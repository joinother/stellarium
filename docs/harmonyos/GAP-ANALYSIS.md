...
| 维度 | 估算 | 说明 |
|------|------|------|
| C++ 桥接命令 | ~96%（172+/178） | 核心命令齐全，Speech/脚本录制/视频录制命令已加；仍缺轨迹回放专用命令 |
| ArkUI 面板 | ~93% | 14 个面板核心完成，脚本面板已补齐（含视频录制区） |
| 桌面版核心功能 | ~93% | 搜索/选星/时间/位置/图层/投影/脚本/插件/帮助/朗读文本/脚本录制/视频帧序列/**切换观测星球（天空+地景整体重算）** |
| 桌面版完整功能 | ~76% | Speech 部分实现（文本预览✅，音频 TTS 受 SDK 限制），脚本录制/视频帧序列已完整，切换观测星球已完整，仍缺轨迹回放/高级选择/天文摄影 |
| **用户可见差距** | **26%** | 普通用户最需要：星表下载、书签、卫星、Oculars、帮助、脚本录制、视频录制已补齐 |
