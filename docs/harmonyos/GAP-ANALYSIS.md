...
| 48 | ~~虚拟指星笔目标端（多设备联动）~~ ✅ 已实现 | 模拟器位置面板截图：输入高度角/方位角 → 星图转向 → 返回命中/空结果 | 手表/多屏联动「指到哪就显示哪颗星」需要目标端收口逻辑 | ~~P0（路线图源）~~ 完成 | 新增 `pointAtSky` 桥（AltAz 高度角/方位角 → `spheToRect(M_PI-az,alt)` → `altAzToJ2000` → `setViewDirectionJ2000`）+ 下一帧 `app.update` 后在屏幕中心 `findAndSelect` 选中天体；位置面板新增「虚拟指星笔测试」区（高度角/方位角输入 + 示例：天顶 + 即时结果回显 + `flashHint` 提示）；`getSelectedObjectInfo` 读取选中天体名称/类型；模拟器端到端验证通过 |

## 四、完成度估算

| 维度 | 估算 | 说明 |
|------|------|------|
| C++ 桥接命令 | ~98%（175+/179） | 核心命令齐全，Speech/脚本录制/视频录制/放大时地景淡出/虚拟指星笔命令已加；仍缺轨迹回放专用命令 |
| ArkUI 面板 | ~95% | 14 个面板核心完成，脚本面板已补齐（含视频录制区），图层面板新增「放大时地景淡出」开关，位置面板新增「虚拟指星笔测试」区 |
| 桌面版核心功能 | ~94% | 搜索/选星/时间/位置/图层/投影/脚本/插件/帮助/朗读文本/脚本录制/视频帧序列/**切换观测星球（天空+地景整体重算）**/**放大时地景淡出** |
| 桌面版完整功能 | ~78% | Speech 部分实现（文本预览✅，音频 TTS 受 SDK 限制），脚本录制/视频帧序列/切换观测星球/放大时地景淡出/虚拟指星笔目标端已完整，仍缺轨迹回放/高级选择/天文摄影 |
| **用户可见差距** | **23%** | 普通用户最需要：星表下载、书签、卫星、Oculars、帮助、脚本录制、视频录制、放大时地景淡出、虚拟指星笔已补齐 |
