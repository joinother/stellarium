...
The same editor can also be opened directly with \key{F7}. It uses the
actions registered by the core and by loaded plugins, so plugin shortcuts
are managed together with the built-in commands. The search field accepts
an action name, an action identifier, a group name, or a displayed shortcut.
The Help Tab (opened with \key{F1}) shows the currently assigned shortcuts;
commands without an assigned key are available from the editor.
